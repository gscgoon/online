<template>
  <div class="main">
    <div class="title">
      <a href="/login">登录</a>
      <span>·</span>
      <a class="active" href="/regist">注册</a>
    </div>
  ...
  export default {
    ...
    methods: {
        ...
        submitRegister() {
            ...
            if (valid) {
                regist.registMember(this.registerVo).then(response => {
                //提示信息
                this.$message({
                    type: 'success',
        ...
        getCodeFun() {
            ...
            if (errMsg == '') {
                regist.sendCodeToMobile(this.registerVo.mobile).then(res => {
                this.sending = false;
                this.timeDown();
                });
            }
